...
bool bsp( Point const a, Point const b, Point const c, Point const point)
{
	...
	if (checkSignEquality(outer_product(vec_ab, vec_bc), outer_product(vec_ab, vec_ap)) == false)
		return (false);
	else if (checkSignEquality(outer_product(vec_bc, vec_ca), outer_product(vec_bc, vec_bp)) == false)
		return (false);
	else if (checkSignEquality(outer_product(vec_ca, vec_ab), outer_product(vec_ca, vec_cp)) == false)
		return (false);
	return (true);
}
